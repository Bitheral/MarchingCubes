...
/*
 * Updating this to use TypeScript, and tidy up the code a bit.
 * @author Bitheral
 * @date 26/11/2022
 */
...
class Grad {
    private gradient: Vector3;
    constructor(x: number, y: number, z: number) {
        this.gradient = new Vector3(x, y, z);
    }

    dot2(x: number, y: number): number {
        return this.gradient.x*x + this.gradient.y*y;
    }

    dot3(x: number, y: number, z: number): number {
        return this.gradient.x*x + this.gradient.y*y + this.gradient.z*z;
    }
}
...
// 2D simplex noise
export function simplex2(offset: Vector2): number {
...
// 2D Perlin Noise
export function perlin2(offset: Vector2) {
    var x = offset.x;
    var y = offset.y;
...
// 3D Perlin Noise
export function perlin3(offset: Vector3) {
    var x = offset.x;
    var y = offset.y;
    var z = offset.z;
